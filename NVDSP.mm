...
- (void) filterContiguousData: (float *)data numFrames:(UInt32)numFrames channel:(UInt32)channel {
    ...
    //NSLog(@"in filterContigousData before filter data is %f", *data);
    // Provide buffer for processing
    float tInputBuffer[numFrames + 2];
    float tOutputBuffer[numFrames + 2];
    
    // Copy the data
    memcpy(tInputBuffer, gInputKeepBuffer[channel], 2 * sizeof(float));   //copying input audio data to tInputBuffer
    memcpy(tOutputBuffer, gOutputKeepBuffer[channel], 2 * sizeof(float)); //copying outp
    ...
    //NSLog(@"inputKeepBuffer Audio data after Filter applied is %f", *gInputKeepBuffer[channel]);
    //NSLog(@"Outputkeepbuffer Audio data after Filter applied is %f", *gOutputKeepBuffer[channel]);
}
